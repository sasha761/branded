...
const defaultConfig = {
  ...
  load(element) {
    ...
    if (element.nodeName.toLowerCase() === 'picture' && element.className === 'js-lazy-load') {
      let img = element.querySelector('img')
      let append = false

      if (element.querySelectorAll('source').length) {
      ...
      if (element.getAttribute('width') || element.getAttribute('height')) {
        img.width = element.getAttribute('width');
        img.height = element.getAttribute('height');
      }
